// src/components/PoolServices.js

import React, { useState } from 'react';
import ServiceCard from './ServiceCard';
import Pagination from './Pagination';
import { poolServices } from '../servicesData';
...
const PoolServices = () => {
  ...
  const filteredServices = poolServices.filter(service => {
    return (
  ...
  const allLocations = Array.from(new Set(poolServices.map(service => service.location)));
    ...
    <div>
      ...
      <div className="services-container">
        {currentServices.map(service => (
          <ServiceCard key={service.id} service={service} category="pool" />
        ))}
      </div>
      <Pagination
