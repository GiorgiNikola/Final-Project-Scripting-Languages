// src/components/FitnessServices.js

import React, { useState } from 'react';
import ServiceCard from './ServiceCard';
import Pagination from './Pagination';
import { fitnessServices } from '../servicesData';
...
const FitnessServices = () => {
  ...
  const filteredServices = fitnessServices.filter(service => {
    return (
  ...
  const allLocations = Array.from(new Set(fitnessServices.map(service => service.location)));
    ...
    <div>
      ...
      <div className="services-container">
        {currentServices.map(service => (
          <ServiceCard key={service.id} service={service} category="fitness" />
        ))}
      </div>
      <Pagination
